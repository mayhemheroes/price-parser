#!/usr/bin/env bash
#
# price-parser/mayhem/test.sh — RUN price-parser's own pytest suite and emit a CTRF summary.
# exit 0 iff no test failed.
#
# PATCH-grade behavioral oracle: tests/test_price_parsing.py is hundreds of parametrized
# known-answer cases asserting Price.fromstring(<raw>).amount / .currency / .currency_raw
# EXACTLY (plus strict xfail cases for known-wrong inputs). A no-op / "exit(0)" / behavior-
# altering patch to price_parser cannot pass it.
#
# The suite is run through the `/mayhem/price-tests` ELF launcher (which exec()s
...
# executable. The system `python3` under /usr/bin is SPARED, but `/mayhem/price-tests` is not —
...
if [ ! -x /mayhem/price-tests ]; then
  echo "FATAL: /mayhem/price-tests launcher missing — run mayhem/build.sh first" >&2
  emit_ctrf "pytest" 0 1 0
  exit 2
fi

echo "=== running pytest (price-parser known-answer suite) via /mayhem/price-tests ==="
out="$(/mayhem/price-tests 2>&1)"; rc=$?
echo "$out"
